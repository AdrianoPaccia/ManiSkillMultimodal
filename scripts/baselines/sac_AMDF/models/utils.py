...
class ImgToLatentModel(nn.Module):
    ...
    def __init__(self, input_space, mode, frames=2, output_dim=256, num_ensemble=32):
        super(ImgToLatentModel, self).__init__()
        self.num_ensemble = num_ensemble
        layers = [2, 2, 2, 2]
        self.encoder = CNN(frames, Z_DIM) if mode in ['rgb', 'depth', 'segmentation'] else MLP(input_space.shape[0], Z_DIM)
        #self.model = ResNet(ResidualBlock, layers, input_space)
        self.linear1 = torch.nn.Linear(256, 2048)
        self.linear2 = torch.nn.Linear(2048, 1024)
        self.bayes1 = LinearFlipout(in_features=1024, out_features=512)
        self.bayes2 = LinearFlipout(in_features=512, out_features=output_dim)
...
class DecoderModel(nn.Module):
    ...
    def __init__(self, latent_dim, mode, output_dim=(256,), num_ensemble=32): # num_ensemble, dim_x, dim_gt):
        super(DecoderModel, self).__init__()
        self.num_ensemble = num_ensemble
        self.mode = mode
        self.output_dim = output_dim

        if mode in ["rgb", "depth", "segmentation"]:
            # Transposed convolution layers to decode the vector into an image

            self.fc = nn.Sequential(
                nn.Linear(latent_dim, 256 * (output_dim[1] // 4) * (output_dim[2] // 4)),
                nn.ReLU())
            self.decoder = nn.Sequential(
                nn.ConvTranspose2d(256, 128, kernel_size=4, stride=2, padding=1),  # Upsample to 16x16
                nn.BatchNorm2d(128),
                nn.ReLU(True),
                nn.ConvTranspose2d(128, 64, kernel_size=4, stride=2, padding=1),  # Upsample to 32x32
                nn.BatchNorm2d(64),
                nn.ReLU(True),
                nn.ConvTranspose2d(64, output_dim[0], kernel_size=3, stride=1, padding=1),  # Final image size
                nn.Tanh()  # Output range should be between -1 and 1 for images
            )
        else:
            self.fc = nn.Sequential(
    ...
    def forward(self, z):
        ...
        z = rearrange(z, "bs k dim -> (bs k) dim")
        h = self.fc(z)

        if self.mode in ["rgb", "depth", "segmentation"]:
            h = h.view(h.size(0), 256, self.output_dim[1] // 4, self.output_dim[1] // 4)

            state = self.decoder(h)
            state = rearrange(
                state, "(bs k) f w h -> bs k f w h", bs=batch_size, k=self.num_ensemble
            )
        else:
            state = self.decoder(h)
