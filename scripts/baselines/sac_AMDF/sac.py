    ...
    while global_step < args.total_timesteps:
        ...
        if args.eval_freq > 0 and (global_step - args.training_freq) // args.eval_freq < global_step // args.eval_freq:
            ...
            for _ in range(args.num_eval_steps):
                with torch.no_grad():
                    ...
                    output = model((process_obs_dict(eval_obs, old_eval_obs, args.modes, device), args.modes), z_old)
        ...
        for local_step in range(args.steps_per_env):
            ...
            if not learning_has_started:
                ...
                output = model((process_obs_dict(obs, old_obs, args.modes, device), args.modes), z_old)
...
if __name__ == "__main__":
    ...
    with open('/home/adriano/Desktop/ManiSkillMultimodal/scripts/baselines/sac_AMDF/config/KITTI_v1.yaml', 'r') as file:
        data = yaml.safe_load(file)
    conf = SimpleNamespace(**data['train'])
    model = KITTI_latent_model(
        env=envs,
        modes=args.modes,
        num_ensemble=conf.num_ensemble,
        win_size=conf.win_size,
        dim_x=256, #conf.dim_x,
        dim_z=conf.dim_z,
        dim_gt=(2,128,128),
    ...
    support_model = AuxiliaryStateModel(
        input_dim=conf.dim_gt,
        output_dim=conf.dim_x,
        num_ensemble=conf.num_ensemble
    ).to(device)
    ...
    rb = ReplayBuffer(
        state_shape=(frames,*envs.single_observation_space.spaces['sensor_data'][envs.data_source][main_mode].shape[:2]),
