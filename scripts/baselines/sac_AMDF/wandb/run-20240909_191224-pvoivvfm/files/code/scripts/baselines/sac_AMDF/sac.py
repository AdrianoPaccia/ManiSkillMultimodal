...
from models.model import latent_model
...
def train(**kwargs):
    ...
    criterion = nn.MSELoss()  # nn.MSELoss() or nn.L1Loss()
    ...
    while global_step < args.total_timesteps:
        ...
        if args.eval_freq > 0 and (global_step - args.training_freq) // args.eval_freq < global_step // args.eval_freq:
            ...
            z_old = support_model(process_obs_dict(eval_obs, old_eval_obs, (args.main_mode,), device)[0].unsqueeze(1))
            ...
            eval_obs, _, _, _, _ = eval_envs.step_mm(action)
            old_action = torch.from_numpy(action).float().to(device)
            ...
            for _ in range(args.num_eval_steps):
                with torch.no_grad():
                    #output = (actual_state, mean_actual_state, atten, pred_states, state_new)
                    output = model(
                        (process_obs_dict(eval_obs, old_eval_obs, args.modes, device), args.modes),
                        z_old,
                        old_action,
                    )
                    z_obs = torch.mean(output[-1], dim=1) # -> final estimation
                    action = actor.get_eval_action(z_obs)
                    ...
                    z_old = torch.cat((z_old[:, :, 1:, :], output[-1].unsqueeze(2)), axis=2)
        ...
        state = process_obs_dict(obs, old_obs, (args.main_mode,), device)[0]
        ...
        for local_step in range(args.steps_per_env):
            ...
            if not learning_has_started:
                ...
                output = model(
                    (process_obs_dict(obs, old_obs, args.modes, device),
                    args.modes),
                    z_old,
                    old_action
                )
                z_obs = torch.mean(output[-1], dim=1)
                z_old = torch.cat((z_old[:, :, 1:, :], output[-1].unsqueeze(2)), axis=2)
                actions, _, _ = actor.get_train_action(z_obs)
            ...
            next_state = process_obs_dict(next_obs, obs, (args.main_mode,), device)[0]
        ...
        for local_update in range(args.grad_steps_per_iteration):
            print(f'Local update: {local_update}', end=('\r'))
            global_update += 1
            data = rb.sample(args.batch_size)

            latent_state = support_model(data.states.unsqueeze(1))
            output = model((data.obs, args.modes), latent_state, data.old_actions)
            z = torch.mean(output[-1], dim=1)

            next_latent_state = support_model(data.states.unsqueeze(1))
            next_output = model((data.next_obs, args.modes), next_latent_state, data.actions)
            next_z = torch.mean(next_output[-1], dim=1)

            # update the value networks
            with torch.no_grad():
                next_state_actions, next_state_log_pi, _ = actor.get_train_action(next_z.detach())
                qf1_next_target = qf1_target(next_z.detach(), next_state_actions)
                qf2_next_target = qf2_target(next_z.detach(), next_state_actions)
                min_qf_next_target = torch.min(qf1_next_target, qf2_next_target) - alpha * next_state_log_pi
                next_q_value = data.rewards.flatten() + (1 - data.dones.flatten()) * args.gamma * (min_qf_next_target).view(-1)
                # data.dones is "stop_bootstrap", which is computed earlier according to args.bootstrap_at_done

            qf1_a_values = qf1(z.detach(), data.actions).view(-1)
            qf2_a_values = qf2(z.detach(), data.actions).view(-1)
            ...
            if global_update % args.policy_frequency == 0:  # TD 3 Delayed update support
                pi, log_pi, _ = actor.get_train_action(z)
                qf1_pi = qf1(z, pi)
                qf2_pi = qf2(z, pi)
                ...
                loss_1 = criterion(next_output[1], data.next_states)  ##gt=image
                loss_p = criterion(next_output[-2][-1], data.next_states)  ##gt=image
                add1_loss = 0
                add2_loss = 0

                for j in range(len(next_output[-2]) - 1):
                    add1_loss = add1_loss + criterion(next_output[-2][j], data.next_states)
                    add2_loss = add2_loss + criterion(next_output[-2][j], next_output[-2][j + 1])
                    representation_loss = loss_1 + loss_p + add1_loss + 0.1 * add2_loss
                ...
                if args.autotune:
                    with torch.no_grad():
                        _, log_pi, _ = actor.get_train_action(z)
...
if __name__ == "__main__":
    ...
    print('... engine setup', end='\r')

    max_action = float(envs.single_action_space.high[0])

    with open('/home/adriano/Desktop/ManiSkillMultimodal/scripts/baselines/sac_AMDF/config/model.yaml', 'r') as file:
        data = yaml.safe_load(file)
    conf = SimpleNamespace(**data)
    dim_gt = (2,*envs.single_observation_space.spaces['sensor_data'][envs.data_source][args.main_mode].shape[:2])
    ...
    model = latent_model(
        ...
        dim_z=Z_DIM,
        dim_a=np.prod(envs.single_action_space.shape),
        dim_gt=dim_gt,
        sensor_len=conf.sensor_len,
    ).to(device)

    support_model = AuxiliaryStateModel(
        input_dim=dim_gt,
        output_dim=Z_DIM,
        num_ensemble=args.num_ensemble
    ).to(device)
    ...
    actor = Actor(Z_DIM, envs).to(device)
    qf1 = SoftQNetwork(input_dim=Z_DIM, output_dim=envs.single_action_space.shape[0]).to(device)
    qf2 = SoftQNetwork(input_dim=Z_DIM, output_dim=envs.single_action_space.shape[0]).to(device)
    qf1_target = SoftQNetwork(input_dim=Z_DIM, output_dim=envs.single_action_space.shape[0]).to(device)
    qf2_target = SoftQNetwork(input_dim=Z_DIM, output_dim=envs.single_action_space.shape[0]).to(device)
    ...
    rb = ReplayBuffer(
        state_shape=(frames,*envs.single_observation_space.spaces['sensor_data'][envs.data_source][args.main_mode].shape[:2]),
