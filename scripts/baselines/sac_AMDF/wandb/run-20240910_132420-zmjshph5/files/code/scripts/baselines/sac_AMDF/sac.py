...
def train(**kwargs):
    ...
    while global_step < args.total_timesteps:
        ...
        if args.eval_freq > 0 and (global_step - args.training_freq) // args.eval_freq < global_step // args.eval_freq:
            ...
            z_old = support_model(eval_envs.get_state().unsqueeze(1).to(device))
        ...
        z_old = support_model(state.unsqueeze(1).to(device))
        ...
        for local_step in range(args.steps_per_env):
            ...
            if not learning_has_started:
                ...
                z_old = torch.cat((z_old[:, :, 1:, :], output[-1].unsqueeze(2)), axis=2)
                actions, _, _ = actor.get_train_action(z_obs).detach()
...
if __name__ == "__main__":
    ...
    eval_envs = build_eval_env(
        ...
        obs_mode="rgb+depth+segmentation", #"rgb+depth+segmentation", #"sensor_data", #,"all",
    ...
    qf1 = SoftQNetwork(input_dim=dim_state[0], output_dim=envs.single_action_space.shape[0]).to(device)
    qf2 = SoftQNetwork(input_dim=dim_state[0], output_dim=envs.single_action_space.shape[0]).to(device)
    qf1_target = SoftQNetwork(input_dim=dim_state[0], output_dim=envs.single_action_space.shape[0]).to(device)
    qf2_target = SoftQNetwork(input_dim=dim_state[0], output_dim=envs.single_action_space.shape[0]).to(device)
