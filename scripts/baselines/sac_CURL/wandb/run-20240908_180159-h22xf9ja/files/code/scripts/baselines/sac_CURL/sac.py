...
from utils import Args, get_distance, compute_SimCLR_loss
    ...
    while global_step < args.total_timesteps:
        ...
        for local_update in range(args.grad_steps_per_iteration):
            ...
            if global_update % args.policy_frequency == 0:  # TD 3 Delayed update support
                ...
                augment_obs = transform(data.obs, augment_trans)
                #eval_obs = transform(data.obs, obs_trans)
                ...
                augment_z, augment_z_mom = actor.get_representations(augment_obs)
                distances = [
                    [cs(z, z_mom) for j, z_mom in enumerate(augment_z_mom)]
                    for i, z in enumerate(augment_z)
                ]
                temp=0.1  #temperature
                for i, di in enumerate(distances):
                    for j, dij in enumerate(distances[i]):
                        d_tot = sum([torch.exp(d_)/temp for d_ in di])
                        #intra similarity
                        if i==j:
                            loss_ii = -torch.log(torch.exp(dij)/temp)/d_tot
                            intra_losses.append(torch.mean(loss_ii))
                        # inter similarity
                        else:
                            loss_ij = - torch.log(torch.exp(dij)/temp)/d_tot
                            inter_losses.append(torch.mean(loss_ij))

                for c, l in zip(inter_coeff+intra_coeff, inter_losses+inter_losses):
                    representation_loss += c*l
        ...
        if (global_step - args.training_freq) // args.log_freq < global_step // args.log_freq:
            ...
            try:
                ...
                writer.add_scalar("losses/inter_modality_loss", sum(inter_losses)/len(inter_losses).item(), global_step)
                writer.add_scalar("losses/intra_modality_loss", sum(intra_losses)/len(intra_losses).item(), global_step)
            except:
                pass
