...
#scripts.environments.wrappers import (EnvMultimodalWrapper)
    ...
    while global_step < args.total_timesteps:
        ...
        if args.eval_freq > 0 and (global_step - args.training_freq) // args.eval_freq < global_step // args.eval_freq:
            ...
            z_img, z_obs = actor.get_encodings((
                torch.cat([
                    image_preprocess(old_eval_img, resize_shape=image_reshape),
                    image_preprocess(eval_img, resize_shape=image_reshape)
                ],dim=1).float(),
                eval_obs
            ))
            z_old = (z_img+z_obs)/2
        ...
        for local_step in range(args.steps_per_env):
            ...
            img_stack = torch.cat([
                image_preprocess(old_img, resize_shape=image_reshape),
                image_preprocess(img, resize_shape=image_reshape)
            ], dim=1)
            ...
            next_img_stack = torch.cat([
                image_preprocess(img, resize_shape=image_reshape),
                image_preprocess(next_img, resize_shape=image_reshape)
            ], dim=1)
...
if __name__ == "__main__":
    ...
    envargs = {
        'game':'manipulation',
        'image_shape': (2, 150, 150),
        'image_noise': {'frequency':0.0, 'types':['nonoise']},
        'conf_noise': {'frequency': 0.0, 'types': ['nonoise']}
    }
    envs = EnvMultimodalWrapper(envs, **envargs)
    eval_envs = EnvMultimodalWrapper(eval_envs, **envargs)
