...
class ReplayBufferMultimodal:
    def __init__(self, env, num_envs: int, buffer_size: int, storage_device: torch.device, sample_device: torch.device):
        ...
        self.obs = torch.zeros((buffer_size, num_envs) + obs_space.shape).to(storage_device)
        self.img = torch.zeros((buffer_size, num_envs) + img_space.shape).to(storage_device)
        self.next_obs = torch.zeros((buffer_size, num_envs) + obs_space.shape).to(storage_device)
        self.next_img = torch.zeros((buffer_size, num_envs) + img_space.shape).to(storage_device)
        self.actions = torch.zeros((buffer_size, num_envs) + env.single_action_space.shape).to(storage_device)
        self.logprobs = torch.zeros((buffer_size, num_envs)).to(storage_device)
        self.rewards = torch.zeros((buffer_size, num_envs)).to(storage_device)
        self.dones = torch.zeros((buffer_size, num_envs)).to(storage_device)
    ...
    def add(self, obs: torch.Tensor, img: torch.Tensor, next_obs: torch.Tensor, next_img: torch.Tensor,
        ...
        self.obs[self.pos] = obs
        self.next_obs[self.pos] = next_obs
        self.img[self.pos] = img
        self.next_img[self.pos] = next_img
        self.actions[self.pos] = action
        self.rewards[self.pos] = reward
        self.dones[self.pos] = done
